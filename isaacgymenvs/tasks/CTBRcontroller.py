...
class CTRBctrl():
    def __init__(self, num_envs, device) -> None:
        ...
        self.body_drone_angvels = torch.zeros((num_envs, 3), device=self.device, dtype=torch.float32)
    ...
    def update(self, actions, drone_quats, drone_angvels, drone_linvels):
        ...
        if len(thrust_offset_idx) > 0:
            self.thrust[thrust_offset_idx] -= thrust_offset_lower[thrust_offset_idx, None].repeat(
                1, self.thrust.shape[1]
            )

        thrust_offset_upper = torch.max(self.thrust, dim=-1)[0] - self.real_thrust_upbound
        thrust_offset_idx2 = torch.where(thrust_offset_upper > 0.0)[0]
        if len(thrust_offset_idx2) > 0:
            self.thrust[thrust_offset_idx2] -= thrust_offset_upper[thrust_offset_idx2, None].repeat(
                1, self.thrust.shape[1]
            )

        # Clamp at the end if not inside the feasible thrust range
        self.thrust[:] = torch.clamp(self.thrust, self.real_thrust_lowbound, self.real_thrust_upbound)
...
if __name__ == "__main__":
    ...
    nn = Model.mlp()
    friction = torch.zeros((num_envs, bodies_per_env, 3), device=device, dtype=torch.float32)
    ...
    for _ in range(sim_steps):
        ...
        friction[:, drone_handle, :] = -0.02*torch.sign(body_drone_linvels)*body_drone_linvels**2
