...
def generate_line_fold_points(num_points):
    t = np.linspace(0, 2, num_points)
    x = t
    y = np.array([0.5, -0.5] * (len(x) // 2))
    z = np.ones_like(t)
    return x, y, z


def save_to_csv(x, y, z, filename='isaacgymenvs/tasks/trajectory/line_fold.csv'):
...
def plot_3d_trajectory(x, y, z):
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('line_fold', fontsize=16)
    ax.legend()
    plt.savefig('isaacgymenvs/tasks/trajectory/figure/line_fold.png', dpi=600)
    plt.show()


num_points = 10
x, y, z = generate_line_fold_points(num_points)
