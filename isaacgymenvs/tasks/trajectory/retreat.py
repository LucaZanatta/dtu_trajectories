...
def generate_retreat_points():
    ...
    t3 = np.linspace(0.4, -0.5, 10)
    x3 = t3
    y3 = np.zeros_like(x3)
    z3 = np.ones_like(x3) + 2*radius
...
def save_to_csv(x, y, z, filename='isaacgymenvs/tasks/trajectory/retreat.csv'):
...
def plot_3d_trajectory(x, y, z):
    ...
    ax.legend()
    plt.savefig('isaacgymenvs/tasks/trajectory/figure/retreat.png', dpi=600)
    plt.show()
