...
def generate_ellipse_points(num_points):
    t = np.linspace(-np.pi/2, np.pi*3/2, num_points)
    a = 1
    b = 0.5
    x = a * np.cos(t)
    y = b * np.sin(t) + b
...
def save_to_csv(x, y, z, filename='isaacgymenvs/tasks/trajectory/ellipse.csv'):
...
def plot_3d_trajectory(x, y, z):
    ...
    ax.legend()
    plt.savefig('isaacgymenvs/tasks/trajectory/figure/ellipse.png', dpi=600)
    plt.show()


num_points = 50
x, y, z = generate_ellipse_points(num_points)
